...
class SQLAInterface(BaseInterface):
    ...
    def query(
        ...
        try:
            count = query_count.order_by(None).scalar()
        except MultipleResultsFound as e:
            print(e)
            # print(query.subquery().columns)
            # q_union_cnt = query(func.sum(query.subquery().columns.anon_1_count_1).label("total_cnt"))
            # count = q_union_cnt.scalar()
            count_array = query_count.order_by(None).all()
            count = 0
